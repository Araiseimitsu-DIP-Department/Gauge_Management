...
| 貸出ID | `pin_gauge_lending.id`（`GENERATED BY DEFAULT AS IDENTITY`。新規貸出登録時は DB が自動採番） |

貸出新規登録（PostgreSQL）では `id` を INSERT 文に含めず、シーケンス `pin_gauge_lending_id_seq` による自動採番に任せます。登録後の一覧表示は、従来どおり貸出日・機番・担当者で再取得します。
